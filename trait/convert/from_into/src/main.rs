...
fn test2() {
    ...
    // let n = 1u32.into() as Length;  // can not infer type for T
    //
    // On stackoverflow => Using .into() when type inference is impossible
    // I don't think there is a better way. Since the type parameter is on the trait, 
    // not the method into(), the turbofish operator into::<i32>() doesn't work. 
    // As you said, you can make it work by using the fully-qualified-syntax:

    let n = Into::<Length>::into(1u32);

    //
    // no auto implemented from
